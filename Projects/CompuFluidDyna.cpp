  ...
  for (int x= 0; x < nbX; x++) {
    for (int y= 0; y < nbY; y++) {
      for (int z= 0; z < nbZ; z++) {
        ...
        DensAdd[x][y][z]= 0.0f;
        if (Force[x][y][z] != 0) {
          VelXAdd[x][y][z]= (float)Force[x][y][z] * D.param[CoeffForceX_________].Get();
          VelYAdd[x][y][z]= (float)Force[x][y][z] * D.param[CoeffForceY_________].Get();
          VelZAdd[x][y][z]= (float)Force[x][y][z] * D.param[CoeffForceZ_________].Get();
        }
        if (Sourc[x][y][z] != 0) {
          DensAdd[x][y][z]= (float)Sourc[x][y][z] * D.param[CoeffSource_________].Get();
        }
      }
    }
  }

  VelocityStep(Bound, std::max((int)std::round(D.param[GaussSeiderIter_____].Get()), 0), D.param[TimeStepSize________].Get(),
               D.param[CoeffViscosity______].Get(), VelXAdd, VelYAdd, VelZAdd, VelXCur, VelYCur, VelZCur);
  DensityStep(Bound, std::max((int)std::round(D.param[GaussSeiderIter_____].Get()), 0), D.param[TimeStepSize________].Get(),
              D.param[CoeffDiffusion______].Get(), VelXCur, VelYCur, VelZCur, DensAdd, DensCur);
...
void CompuFluidDyna::DiffuseField(
    const std::vector<std::vector<std::vector<int>>>& iType, const bool iMirror,
...
void CompuFluidDyna::ProjectField(
...
void CompuFluidDyna::DensityStep(
    const std::vector<std::vector<std::vector<int>>>& iType,
    const int iIter, const float iTimeStep, const float iDiffusionCoeff,
    const std::vector<std::vector<std::vector<float>>>& iVelX,
    const std::vector<std::vector<std::vector<float>>>& iVelY,
...
void CompuFluidDyna::VelocityStep(
    const std::vector<std::vector<std::vector<int>>>& iType,
    const int iIter, const float iTimeStep, const float iDiffusionCoeff,
