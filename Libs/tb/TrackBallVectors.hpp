...
public:
    ...
    inline float dot(const vec2& v) const { return x[0] * v.x[0] + x[1] * v.x[1]; }
...
public:
    ...
    inline float length() const { return sqrt(length2()); }
    inline float length2() const { return x[0] * x[0] + x[1] * x[1] + x[2] * x[2]; }
...
public:
    ...
    inline float length() const { return sqrt(length2()); }
    inline float length2() const { return x[0] * x[0] + x[1] * x[1] + x[2] * x[2] + x[3] * x[3]; }
...
public:
    ...
    inline float dot(const vecn<N>& v) const {
      float r= 0.0;
      for (unsigned int i= 0; i < N; ++i) r+= (x[i] * v.x[i]);
      return r;
    }
    inline float length() const { return sqrt(length2()); }
    ...
    friend vecn<N> operator-(const vecn<N>& w, const vecn<N>& v) { return vecn<N>(w)-= v; }
    friend vecn<N> operator*(const vecn<N>& v, float f) { return vecn<N>(v)*= f; }
    friend vecn<N> operator*(float f, const vecn<N>& v) { return vecn<N>(v)*= f; }
    friend float operator*(const vecn<N>& a, const vecn<N>& b) { return a.dot(b); }
