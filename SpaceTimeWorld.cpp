...
void SpaceTimeWorld::Init() {
  ...
  if (loadedImage.empty())
    SrtFileInput::LoadImagePNGFile("Albert.png", loadedImage, true);
  ...
  for (int h= 0; h < screenNbH; h++) {
    for (int v= 0; v < screenNbV; v++) {
      for (int s= 0; s < screenNbS - 1; s++) {
        ...
        photonVel[h][v][s + 1]= photonVel[h][v][s] + D.param[GR_CurvFactor_______].val * worldFlows[idxT][idxX][idxY][idxZ] / double(screenNbS);
        screenCount[h][v]++;
